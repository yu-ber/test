...
<div class="page-container">
	...
	<shiro:hasPermission name="admin:delete">
		<a href="javascript:;" onclick="deletes()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
 	</shiro:hasPermission>
...
function operationFormatter(value,row,index){
	...
	html +="<span onclick='admin_edit("+value+");' style='color: green;cursor: pointer;' class='glyphicon glyphicon-pencil'></span>&nbsp;&nbsp;&nbsp;&nbsp";
	html +="</shiro:hasPermission>";
	html +="<shiro:hasPermission name='admin:delete'>";
	html +="<span onclick='admin_del("+value+");' style='color: black;cursor: pointer;' class='glyphicon glyphicon-trash'></span>";
	html +="</shiro:hasPermission>";
	return html;			
}
...
/*管理员-增加*/
function admin_add(){
	layer_show("管理员添加","admin/edit.do",800,500);
}

/*管理员-编辑*/
function admin_edit(userId){
	layer_show("管理员修改","admin/edit.do?userId="+userId,800,500);
}
...
function deletes(){
     ...
     layer.confirm('确认要删除吗？',{icon: 3},function(){
 		...
 		$.get("admin/deletes.do?idList="+idList,function(data){
 			//弹出一个提示消息
 			layer.msg(data.msg, {time: 1000, icon:data.code});
 			console.log(idList)
 			//删除成功，刷新一下表格
